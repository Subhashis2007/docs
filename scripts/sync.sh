...
main() {
    ...
    repo sync -c --force-sync --no-clone-bundle --no-tags --prune --retry-fetches=128 2>&1 | tee /tmp/output.txt
 ...
 if ! grep -qe "Failing repos:\|uncommitted changes are present" /tmp/output.txt ; then
    ...
    find .repo -name '*.lock' -delete
    repo sync -c --force-sync --no-clone-bundle --no-tags --prune --retry-fetches=128
}
